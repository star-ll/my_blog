---
{"publish":true,"created":"2025-08-19T21:16:17.561+08:00","modified":"2026-01-31T12:30:40.832+08:00","cssclasses":""}
---
...
即 **timerQueue 越前面的元素 startTime 越小，需要尽早执行，taskQueue 越前面的元素 expirationTime 越小，越需要尽快执行。**
...
可以看到，`shouldYieldToHost` 函数的作用是比较 `getCurrentTime () - startTime`，判断当前时间距离开始时间是否超过了 `frameInterval`，也就是 `5 ms`，如果超过了这个时间 `shouldYieldToHost` 就会返回 `true`，从而中断 `workLoop` 函数中的循环，即**中断**任务队列的执行。
